...
- Appearance is selected through `Overlay Theme` (`ClassicDark`, `Glass`, `Cyber`, `HighContrast`), `Overlay Layout` (`Classic`, `CompactCards`, `DiagnosticsWide`, `OverdrawFocus`), and `Overlay Font` (`Manrope`, `JetBrainsMono`, `LegacyRuntime`) on the setup window `Presets` and `Runtime` tabs.
