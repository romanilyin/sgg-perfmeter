...
- Presets group a display mode and module flags: `Minimal`, `Timing`, `Rendering`, `Memory`, `Overdraw`, `FullDiagnostics`, `AgentDebug`, and `Custom`. `AgentDebug` uses `MetricBars` so its `CpuCores` module is visible. Module flags (`Fps`, `Timing`, `Graphs`, `Rendering`, `SrpBatcher`, `Brg`, `Uploads`, `Memory`, `Gc`, `GpuMemory`, `Overdraw`, `Heatmap`, `Warnings`, `CustomMetrics`, `CpuCores`) filter overlay rows and hide the graph block when `Graphs` is disabled.
...
- In `Full`, visible fields include state, bottleneck, FPS/lows/spikes, CPU/GPU timings, draw calls, SetPass, batches, vertices, SRP/BRG counters, index uploads, overdraw state/progress/ratio, memory, optional Windows/Apple/Android/Linux CPU core load rows in `MetricBars`, up to eight custom metric rows when enabled, and warning text when present. Apple Silicon cores are reported as a flat logical-core list; PerfMeter does not label E/P cores.
