...
- Custom metrics: `IPerfMeterCustomMetricProvider`, `PerfMeterCustomMetricSnapshot`, `PerformanceMeter.RegisterCustomMetricProvider(...)`, `UnregisterCustomMetricProvider(...)`, `ClearCustomMetricProviders()`, and `GetCustomMetrics()` let projects add counters without forking PerfMeter. `PerformanceMeter.GetCpuCoreLoads()` exposes per-core CPU load on Windows 11/Windows Editor through `NtQuerySystemInformation`, Apple platforms through `host_processor_info`, and Android/Linux through `/proc/stat`.
...
- The `Presets` tab creates and edits project-owned JSON settings at `Assets/Resources/SGG.PerfMeter/perfmeter-settings.json`; runtime loads it with `Resources.Load<TextAsset>("SGG.PerfMeter/perfmeter-settings")`. `ScriptableObject` settings are intentionally not used. The tab separates work mode, technical settings, and the visual overlay preset so overlay appearance cannot change target FPS, collection mode, overlay visibility, session/rule/overdraw tunables, or alert cooldowns.
- Visual overlay presets are stored as separate JSON files under `Assets/SGG PerfMeter/Presets/Overlay/`; custom variants live under `Assets/SGG PerfMeter/Presets/Overlay/custom/`. `Tools/SGG PerfMeter/Ensure Default Overlay Presets` creates `fps-only`, `compact-timing`, `classic-cards`, `graphs`, and `full-diagnostics`; saving settings bakes all valid project presets into the Resources JSON for builds without a runtime `AssetDatabase` dependency.
...
- The `Runtime` tab is for Play Mode: buttons are disabled in Edit Mode and grouped by runtime status, runtime controls, technical overrides, visual preset selection, and diagnostics/actions. The Runtime tab only selects/applies visual presets; layout/style/widget composition editing stays on the `Presets` tab.
...
- Layout is absolute with `PickingMode.Ignore`, so it does not intercept game input; in graph modes the overlay is split into a wider graph block on top and a text/side-panel row below.
- The overlay defaults to the top-right corner (`TopRight`), `MetricBars` layout, CPU/GPU graphs, top timing/budget widgets, and the compact `CpuCoreBars` side panel; available corners are `TopLeft`, `TopRight`, `BottomLeft`, and `BottomRight`.
- Display layout is the single runtime display selector. `FpsOnly` shows one FPS/1%/0.1% line, `TextCompact` shows a compact text summary, `Graphs` shows FPS plus CPU/GPU graphs, `Classic` keeps the full text overlay, `CompactCards` / `DiagnosticsWide` / `OverdrawFocus` change the full layout shape, `MetricBars` uses realtime bars, and `Custom` represents combinations that do not match a built-in display preset.
- Appearance is selected through `Overlay Theme` (`ClassicDark`, `Glass`, `Cyber`, `HighContrast`), `Overlay Layout` (`FpsOnly`, `TextCompact`, `Graphs`, `Classic`, `CompactCards`, `DiagnosticsWide`, `OverdrawFocus`, `MetricBars`, `Custom`), and `Overlay Font` (`Manrope`, `JetBrainsMono`, `LegacyRuntime`) on the setup window `Presets` and `Runtime` tabs. `MetricBars` replaces the lower text block with thin realtime bars: min/max flank the bar, current remains colored, the bar carries distinct average/1%/0.1% risk ticks, and the trailing stats show average, 1%, and 0.1% as fixed columns separated by spacing.
- Visual presets describe overlay appearance/composition only: anchor, layout, theme, font, scale, opacity, max width, gap, widget ids, enabled state, order, variant, and height. Legacy `PerfMeterOverlayPreset` / module flags (`Fps`, `Timing`, `Graphs`, `Rendering`, `SrpBatcher`, `Brg`, `Uploads`, `Memory`, `Gc`, `GpuMemory`, `Overdraw`, `Heatmap`, `Warnings`, `CustomMetrics`, `CpuCores`, `CpuCoreBars`, `CpuCoreGraphs`) remain the runtime/API compatibility layer and filter overlay rows/panels. `FullDiagnostics` defaults include `CpuCoreBars`; legacy `CpuCores` rows and `CpuCoreGraphs` stay opt-in.
...
- The right side of the graphs shows fixed-width colored label badges for `frame`, `other`, `main`, `render`, and `gpu`; values sit on a dark strip as current, average, worst 1%, and worst 0.1% timing columns with spacing only.
...
- Target FPS is selected from `15/30/60/90/120/144/240`; the matching `FrameBudgetMs` drives the red target line and target label shown left of the graph plus the scale formula `max(averageTimeMs * 1.1, FrameBudgetMs * 1.2)`.
...
- In `Full`, visible fields include state, bottleneck, FPS/lows/spikes, CPU/GPU timings, draw calls, SetPass, batches, vertices, SRP/BRG counters, index uploads, overdraw state/progress/ratio, memory, optional Windows/Apple/Android/Linux CPU core load rows/panels, up to eight custom metric rows when enabled, and warning text when present. CPU core bars and graph lines use load colors: green below 25%, yellow from 25% through 75%, and red above 75%. Apple Silicon cores are reported as a flat logical-core list; PerfMeter does not label E/P cores.
