...
- Custom metrics: registered `IPerfMeterCustomMetricProvider` instances are called during runtime collection. Provider exceptions become unavailable `PerfMeterCustomMetricSnapshot` entries with `Warning`, so they do not interrupt core metric collection or session recording. The first pass exposes custom metrics through the API, session JSON export, and MCP `perfmeter.metrics.latest`; overlay support is intentionally not included.
