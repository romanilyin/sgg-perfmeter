...
- Вкладка `Runtime` предназначена для Play Mode: в Edit Mode кнопки отключены, а в Play Mode они сгруппированы по runtime status, collection, visibility, target FPS, overlay mode/layout/theme/font/corner, CPU core panels и overdraw. Активные radio/toggle кнопки подсвечиваются желтым фоном.
...
- Alerts: default rules следят за CPU/GPU frame budget, FPS ниже target, unavailable GPU timing и высоким overdraw ratio. Threshold overdraw ratio, consecutive frame counts, structured log, callback, Editor warning cooldowns и toggle `disableEditorWarnings` берутся из JSON settings, чтобы warning в Editor Console можно было throttling'ом ограничить или полностью отключить. `PerfMeterStatusSnapshot` содержит `ActiveAlertCount`, `FiredAlertCount`, `LatestAlertRuleId` и `LatestAlertMessage`.
...
- По умолчанию overlay находится в правом верхнем углу (`TopRight`), режиме `Full`, layout `MetricBars`, с CPU/GPU графиками, верхними timing/budget widgets и компактной боковой панелью `CpuCoreBars`; доступные углы: `TopLeft`, `TopRight`, `BottomLeft`, `BottomRight`.
