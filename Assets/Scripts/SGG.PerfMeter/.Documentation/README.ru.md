...
- Session summary/export: `PerfMeterSessionSummarySnapshot` содержит sample count, dropped sample count, first/last frame, duration, average/min/max frame time и FPS, bottleneck/spike counts, warnings, стартовые settings/device/camera metadata и scene names. `PerfMeterSessionOptions` задает `WarmupFrames`, `SampleIntervalSeconds` и `MaxSamples`. `GetSessionSamples()` возвращает copy массива samples, а `ExportSessionJson(path)` / `ExportSessionCsv(path)` пишут schema/package marker, summary/options/metadata и rows с CPU/GPU/FPS/render/SRP/BRG/upload/memory/overdraw/warning/counter availability metrics.
...
- `perfmeter.session.start` запускает bounded recording и принимает опциональные `warmup_frames`, `sample_interval_seconds`, `max_samples`.
