...
- Вкладка `Presets` создает и редактирует project-owned JSON settings в `Assets/Resources/SGG.PerfMeter/perfmeter-settings.json`; runtime читает его как `Resources.Load<TextAsset>("SGG.PerfMeter/perfmeter-settings")`. `ScriptableObject` настройки не используются. Вкладка разделяет work mode, technical settings и visual overlay preset, чтобы внешний вид overlay не менял target FPS, collection mode, overlay visibility, session/rule/overdraw tunables или alert cooldowns.
- Visual overlay presets хранятся отдельными JSON-файлами в `Assets/SGG PerfMeter/Presets/Overlay/`, пользовательские варианты - в `Assets/SGG PerfMeter/Presets/Overlay/custom/`. Команда `Tools/SGG PerfMeter/Ensure Default Overlay Presets` создает `fps-only`, `compact-timing`, `classic-cards`, `graphs` и `full-diagnostics`; при сохранении settings все валидные project presets baked в Resources JSON для билдов без зависимости runtime от `AssetDatabase`.
...
- Вкладка `Runtime` предназначена для Play Mode: в Edit Mode кнопки отключены, а в Play Mode они сгруппированы по runtime status, runtime controls, technical overrides, visual preset selection и diagnostics/actions. Runtime tab только выбирает/применяет visual presets; редактирование layout/style/widget composition остается во вкладке `Presets`.
...
- Alerts: default rules следят за CPU/GPU frame budget, FPS ниже target, unavailable GPU timing и высоким overdraw ratio. Threshold overdraw ratio, consecutive frame counts, structured log, callback, Editor warning cooldowns и toggle `editorWarningsEnabled` берутся из JSON settings, чтобы warning в Editor Console можно было throttling'ом ограничить или полностью отключить. `PerfMeterStatusSnapshot` содержит `ActiveAlertCount`, `FiredAlertCount`, `LatestAlertRuleId`, `LatestAlertMessage` и `EditorWarningsEnabled`.
...
- По умолчанию overlay находится в правом верхнем углу (`TopRight`), layout `MetricBars`, с CPU/GPU графиками, верхними timing/budget widgets и компактной боковой панелью `CpuCoreBars`; доступные углы: `TopLeft`, `TopRight`, `BottomLeft`, `BottomRight`.
- Display layout является единым runtime selector'ом отображения. `FpsOnly` показывает одну строку FPS/1%/0.1%, `TextCompact` показывает компактную текстовую сводку, `Graphs` показывает FPS и CPU/GPU графики, `Classic` оставляет полный текстовый overlay, `CompactCards` / `DiagnosticsWide` / `OverdrawFocus` меняют форму полного layout, `MetricBars` использует realtime-полоски, а `Custom` означает комбинацию, которая не совпадает со встроенным display preset.
- Оформление выбирается через `Overlay Theme` (`ClassicDark`, `Glass`, `Cyber`, `HighContrast`), `Overlay Layout` (`FpsOnly`, `TextCompact`, `Graphs`, `Classic`, `CompactCards`, `DiagnosticsWide`, `OverdrawFocus`, `MetricBars`, `Custom`) и `Overlay Font` (`Manrope`, `JetBrainsMono`, `LegacyRuntime`) во вкладках `Presets` и `Runtime` окна setup. `MetricBars` заменяет нижний text block тонкими realtime-полосками: min/max стоят по краям бара, current остается цветным, на баре видны отдельные risk ticks для average/1%/0.1%, а справа идут average, 1% и 0.1% фиксированными колонками с пробелами между ними.
- Visual presets описывают только appearance/composition overlay: anchor, layout, theme, font, scale, opacity, max width, gap, widget ids, enabled state, order, variant и height. Legacy `PerfMeterOverlayPreset` / module flags (`Fps`, `Timing`, `Graphs`, `Rendering`, `SrpBatcher`, `Brg`, `Uploads`, `Memory`, `Gc`, `GpuMemory`, `Overdraw`, `Heatmap`, `Warnings`, `CustomMetrics`, `CpuCores`, `CpuCoreBars`, `CpuCoreGraphs`) остаются runtime/API compatibility layer и фильтруют строки/панели overlay. `FullDiagnostics` по умолчанию включает `CpuCoreBars`; legacy `CpuCores` rows и `CpuCoreGraphs` остаются opt-in.
