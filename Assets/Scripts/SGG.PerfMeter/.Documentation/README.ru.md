...
- Оформление выбирается через `Overlay Theme` (`ClassicDark`, `Glass`, `Cyber`, `HighContrast`), `Overlay Layout` (`Classic`, `CompactCards`, `DiagnosticsWide`, `OverdrawFocus`) и `Overlay Font` (`Manrope`, `JetBrainsMono`, `LegacyRuntime`) во вкладках `Presets` и `Runtime` окна setup.
