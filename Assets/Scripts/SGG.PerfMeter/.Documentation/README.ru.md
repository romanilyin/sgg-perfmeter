...
- Presets группируют режим и набор module flags: `Minimal`, `Timing`, `Rendering`, `Memory`, `Overdraw`, `FullDiagnostics`, `AgentDebug` и `Custom`. `AgentDebug` использует `MetricBars`, чтобы модуль `CpuCores` был виден. Module flags (`Fps`, `Timing`, `Graphs`, `Rendering`, `SrpBatcher`, `Brg`, `Uploads`, `Memory`, `Gc`, `GpuMemory`, `Overdraw`, `Heatmap`, `Warnings`, `CustomMetrics`, `CpuCores`) фильтруют строки overlay и скрывают graph block, когда `Graphs` отключен.
...
- В `Full` отображаются state, bottleneck, FPS/lows/spikes, CPU/GPU timings, draw calls, SetPass, batches, vertices, SRP/BRG counters, index uploads, overdraw state/progress/ratio, memory, опциональные строки нагрузки CPU по ядрам на Windows/Apple/Android/Linux в `MetricBars`, до восьми строк custom metrics при включенном модуле и warning text при наличии. На Apple Silicon ядра показываются плоским списком logical cores; PerfMeter не маркирует E/P cores.
