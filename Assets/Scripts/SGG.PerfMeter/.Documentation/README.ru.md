...
- Sessions: `PerformanceMeter.StartSession()`, `PerformanceMeter.StartSession(PerfMeterSessionOptions options)`, `PerformanceMeter.StopSession()`, `PerformanceMeter.GetSessionSummary()` и `PerformanceMeter.IsSessionRecording` управляют bounded in-memory recording без JSON/CSV export в этой версии.
...
- Session summary: `PerfMeterSessionSummarySnapshot` содержит sample count, dropped sample count, first/last frame, duration, average/min/max frame time и FPS, bottleneck/spike counts, warnings, стартовые settings/device/camera metadata и scene names. `PerfMeterSessionOptions` задает `WarmupFrames`, `SampleIntervalSeconds` и `MaxSamples`.
