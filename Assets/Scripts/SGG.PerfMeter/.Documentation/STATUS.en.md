...
- Editor setup actions for agents: `PerfMeterSetupActions.GetStatusReport()`, `PerfMeterSetupActions.EnableFrameTimingStats()`, `PerfMeterSetupActions.InstallRendererFeatures()`, `PerfMeterSetupActions.EnsureDefaultOverlayPresets()`, `PerfMeterSetupActions.CreateDefaultSettings()`, `PerfMeterSetupActions.SaveSettings(...)`, `PerfMeterSetupActions.ApplySettingsToRuntime()`, and `PerfMeterSetupActions.RunRecommendedSetup()`.

## Setup

- Open `SGG/Perfmeter/Setup` to inspect the project, list active/discovered URP renderer assets, install `PerfMeterRenderGraphFeature` into all or selected editable renderers, configure work mode/technical settings/visual presets, inspect overlay widgets on the `Debug` tab, and copy bootstrap code if needed.
- The `Presets` tab creates and edits `Assets/Resources/SGG.PerfMeter/perfmeter-settings.json`; this is project-owned JSON, not a `ScriptableObject`. Visual overlay presets live separately under `Assets/SGG PerfMeter/Presets/Overlay/` and `Assets/SGG PerfMeter/Presets/Overlay/custom/`, then saving settings bakes them into the Resources JSON for builds without a runtime `AssetDatabase` dependency.
- The `Runtime` tab groups Play Mode controls by status, runtime controls, technical overrides, visual preset selection, and diagnostics/actions; visual presets are only selected/applied there, while authoring stays on the `Presets` tab.
- The `Debug` tab lists package overlay widgets and project custom metric providers with source, widget type, module, and implementation details.
- The `Settings` tab selects setup window language from XLIFF files under `Editor/UI/Localization`; localization is Editor-window only and does not change runtime output or generated snippets.
...
- Editor warning alerts have a direct `editorWarningsEnabled` JSON toggle, runtime `Enable Editor Warning Logs` button, and cooldown, so warnings can be disabled or throttled instead of writing every frame; default rule thresholds and consecutive-frame windows are also editable in JSON/Presets.
