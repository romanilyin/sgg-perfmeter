...
- MCP latest metrics output includes `custom_metrics`; session JSON export writes `custom_metrics` in sample rows and `custom_metric_sample_count` in metadata. Custom metric overlay support is not implemented yet.
