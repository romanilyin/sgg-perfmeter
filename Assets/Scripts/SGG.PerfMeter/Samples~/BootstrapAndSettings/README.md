...
Use the component when you want scene-local control over collection mode, overlay preset, corner, and target FPS. Use the JSON file when you want PerfMeter to auto-start from project-owned settings without writing bootstrap code, including overlay scale/refresh/history, alert defaults, session defaults, and overdraw frame limits.
