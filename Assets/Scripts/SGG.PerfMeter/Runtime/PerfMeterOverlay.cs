...
namespace SGG.PerfMeter
{
	...
	internal sealed class PerfMeterOverlay : MonoBehaviour
	{
		...
		private const int MaxCpuCoreRows = PerfMeterCpuCoreSampler.MaxCoreCount;
		...
		private readonly Label[] _frameTimeStripCustomLegends = new Label[PerfMeterOverlayPresetUtility.MaxCustomMetricGraphSeries];
		...
		private VisualElement _customMetricGraphSection;
		private VisualElement _rawFrameTimeSection;
		private VisualElement _frameTimeStripLegendRow;
		...
		private PerfMeterFrameTimeStripElement _customMetricGraph;
		...
		internal int FrameTimeStripCustomSeriesCount => _customMetricGraph?.CustomSeriesCount ?? 0;
		...
		internal void SetCustomMetricGraphs(PerfMeterCustomMetricGraphJson[] configurations)
		{
			_customMetricGraphs = PerfMeterOverlayPresetUtility.CloneCustomMetricGraphs(configurations);
			if (_customMetricGraph != null)
			{
				_customMetricGraph.ConfigureCustomMetricSeries(_customMetricGraphs);
		...
		internal void SetLayoutDescriptor(PerfMeterOverlayPresetJson preset)
		{
			...
			_customMetricGraph?.SetGraphHeight(_frameTimeStripHeight);
		...
		internal void SetTuning(float scale, float opacity, float fontSize, float refreshIntervalSeconds, int graphHistoryLength)
		{
			...
			_customMetricGraph?.SetHistoryCapacity(_graphHistoryLength);
		...
		internal void RecordFrameTimingSample(int collectionFrame, double cpuFrameTimeMs, bool cpuValid, double gpuFrameTimeMs, bool gpuValid)
		{
			...
			bool recordFrameTimeStrip = _frameTimeStrip != null && ShouldShowRawFrameTimeStrip();
		...
		{
			if (!_isVisible || !isActiveAndEnabled || _customMetricGraph == null || !ShouldShowCustomMetricGraph())
			{
				return;
			}

			using PerfMeterSelfObservability.MeasurementScope selfOverheadScope = PerfMeterSelfObservability.Measure(PerfMeterSelfOverheadComponent.Overlay);
			_customMetricGraph.AddCustomMetricSamples(collectionFrame, metrics, count);
		}

		internal bool TryGetFrameTimeStripCustomSample(int series, int sample, out double value, out bool valid)
		{
			value = 0d;
			valid = false;
			return _customMetricGraph != null && _customMetricGraph.TryGetCustomSeriesSample(series, sample, out value, out valid);
		}
		...
		private void ResetVisualReferences()
		{
			...
			_customMetricGraphSection = null;
			_rawFrameTimeSection = null;
			_frameTimeStripLegendRow = null;
			...
			_cpuGraph = null;
			_gpuGraph = null;
			_customMetricGraph = null;
			_frameTimeStrip = null;
			_frameTimeStripStatusLabel = null;
			Array.Clear(_frameTimeStripCustomLegends, 0, _frameTimeStripCustomLegends.Length);
			_cpuCoreBars = null;
			_cpuCoreGraphs = null;
		...
		{
			_customMetricGraphSection = new VisualElement
			{
				name = "sgg-perfmeter-custom-metric-graph-section",
				pickingMode = PickingMode.Ignore
			};
			_customMetricGraphSection.style.flexDirection = FlexDirection.Column;

			VisualElement customHeader = CreateFrameTimeStripHeader("CUSTOM GPU STAGES", "independent millisecond scales");
			_customMetricGraphSection.Add(customHeader);

			_frameTimeStripLegendRow = new VisualElement
			{
				name = "sgg-perfmeter-frame-time-strip-custom-legends",
				pickingMode = PickingMode.Ignore
			};
			_frameTimeStripLegendRow.style.flexDirection = FlexDirection.Row;
			_frameTimeStripLegendRow.style.flexWrap = Wrap.Wrap;
			_frameTimeStripLegendRow.style.display = DisplayStyle.None;
			for (int i = 0; i < _frameTimeStripCustomLegends.Length; i++)
			{
				Label legend = CreateSmallLabel(string.Empty, MutedTextColor, TextAnchor.MiddleLeft, PerfMeterOverlayFontRole.Numeric);
				legend.name = "sgg-perfmeter-frame-time-strip-custom-legend-" + i.ToString(CultureInfo.InvariantCulture);
				legend.style.marginRight = 10f;
				legend.style.display = DisplayStyle.None;
				_frameTimeStripCustomLegends[i] = legend;
				_frameTimeStripLegendRow.Add(legend);
			}
			_customMetricGraphSection.Add(_frameTimeStripLegendRow);

			_customMetricGraph = new PerfMeterFrameTimeStripElement(_graphHistoryLength);
			_customMetricGraph.name = "sgg-perfmeter-custom-metric-graph";
			_customMetricGraph.SetRenderChannels(drawFrameTime: false, drawCustomMetrics: true);
			_customMetricGraph.SetGraphHeight(_frameTimeStripHeight);
			_customMetricGraph.ConfigureCustomMetricSeries(_customMetricGraphs);
			_customMetricGraphSection.Add(_customMetricGraph);
			_frameTimeStripBlock.Add(_customMetricGraphSection);

			_rawFrameTimeSection = new VisualElement
			{
				name = "sgg-perfmeter-raw-frame-time-section",
				pickingMode = PickingMode.Ignore
			};
			_rawFrameTimeSection.style.flexDirection = FlexDirection.Column;
			_rawFrameTimeSection.style.marginTop = 6f;

			VisualElement rawHeader = CreateFrameTimeStripHeader("RAW FRAME TIME", "per collected frame / no smoothing");
			...
			_frameTimeStripStatusLabel.style.flexShrink = 0f;
			rawHeader.Add(_frameTimeStripStatusLabel);
			_rawFrameTimeSection.Add(rawHeader);

			_frameTimeStrip = new PerfMeterFrameTimeStripElement(_graphHistoryLength);
			_frameTimeStrip.SetRenderChannels(drawFrameTime: true, drawCustomMetrics: false);
			_frameTimeStrip.SetGraphHeight(_frameTimeStripHeight);
			_rawFrameTimeSection.Add(_frameTimeStrip);
			_frameTimeStripBlock.Add(_rawFrameTimeSection);
			UpdateFrameTimeStripCustomLegends();
		}

		private static VisualElement CreateFrameTimeStripHeader(string title, string cadence)
		{
			...
			Label titleLabel = CreateSmallLabel(title, FrameColor, TextAnchor.MiddleLeft, PerfMeterOverlayFontRole.Medium);
			titleLabel.style.marginRight = 8f;
			header.Add(titleLabel);
			Label cadenceLabel = CreateSmallLabel(cadence, MutedTextColor, TextAnchor.MiddleLeft);
			cadenceLabel.style.flexGrow = 1f;
			header.Add(cadenceLabel);
			return header;
		}

		private void UpdateFrameTimeStripCustomLegends()
		{
			if (_frameTimeStripLegendRow == null || _customMetricGraph == null)
			{
				return;
			}

			int seriesCount = _customMetricGraph.CustomSeriesCount;
			for (int i = 0; i < _frameTimeStripCustomLegends.Length; i++)
			{
				Label legend = _frameTimeStripCustomLegends[i];
				if (legend == null)
				{
					continue;
				}

				if (i >= seriesCount || !_customMetricGraph.TryGetCustomSeriesConfiguration(i, out PerfMeterCustomMetricGraphConfiguration configuration))
				{
					legend.style.display = DisplayStyle.None;
					continue;
				}
				...
				_valueBuilder.Append(configuration.MetricId).Append(' ');
				if (_customMetricGraph.TryGetCustomSeriesLatest(i, out double latest, out bool latestValid) && latestValid)
				{
					AppendDouble(_valueBuilder, latest, "0.00");
				}
				else
				{
				...
				{
					_valueBuilder.Append(' ').Append(configuration.Unit);
				}
				_valueBuilder.Append(" [");
				AppendDouble(_valueBuilder, configuration.Min, "0.###");
				_valueBuilder.Append("..");
				AppendDouble(_valueBuilder, configuration.Max, "0.###");
				_valueBuilder.Append(']');
				if (Math.Abs(configuration.DisplayScale - 1d) > 0.000001d)
				{
					_valueBuilder.Append(" x");
					AppendDouble(_valueBuilder, configuration.DisplayScale, "0.###");
				}

				legend.text = _valueBuilder.ToString();
				legend.style.color = configuration.Color;
				legend.style.display = DisplayStyle.Flex;
			}

			_frameTimeStripLegendRow.style.display = seriesCount > 0 ? DisplayStyle.Flex : DisplayStyle.None;
		}
		...
		private void ApplyModeLayout()
		{
			...
			bool showCustomMetricGraph = ShouldShowCustomMetricGraph();
			bool showRawFrameTimeStrip = ShouldShowRawFrameTimeStrip();
			bool showFrameTimeStrip = showCustomMetricGraph || showRawFrameTimeStrip;
			...
			if (_customMetricGraphSection != null)
			{
				_customMetricGraphSection.style.display = showCustomMetricGraph ? DisplayStyle.Flex : DisplayStyle.None;
			}
			if (_rawFrameTimeSection != null)
			{
				_rawFrameTimeSection.style.display = showRawFrameTimeStrip ? DisplayStyle.Flex : DisplayStyle.None;
				_rawFrameTimeSection.style.marginTop = showCustomMetricGraph && showRawFrameTimeStrip ? 6f : 0f;
			}
			if (_cpuCoreGraphsBlock != null)
			{
		...
		private bool ShouldShowRawFrameTimeStrip()
		{
			bool widgetEnabled = !_hasLayoutDescriptor || _frameTimeStripWidgetEnabled;
			return widgetEnabled &&
				(_mode == PerfMeterOverlayMode.Graphs || _mode == PerfMeterOverlayMode.Full) &&
				HasModule(PerfMeterOverlayModule.Graphs);
		}

		private bool ShouldShowCustomMetricGraph()
		{
			bool widgetEnabled = !_hasLayoutDescriptor || _frameTimeStripWidgetEnabled;
			return widgetEnabled &&
				(_mode == PerfMeterOverlayMode.Graphs || _mode == PerfMeterOverlayMode.Full) &&
				(HasModule(PerfMeterOverlayModule.CustomMetrics) || HasModule(PerfMeterOverlayModule.Graphs)) &&
				(_customMetricGraph?.CustomSeriesCount ?? 0) > 0;
		}
		...
		{
			if (_frameTimeStripStatusLabel == null || _frameTimeStrip == null || !ShouldShowRawFrameTimeStrip())
			{
				return;
			}
		...
		private void BuildMetricBarText(PerfMeterStatusSnapshot status, PerfMeterMetricsSnapshot metrics, string warning)
		{
			...
			bool hasTiming = HasModule(PerfMeterOverlayModule.Timing);
			if (hasTiming)
			{
			...
			if (HasModule(PerfMeterOverlayModule.CpuCores))
			{
				AppendCpuCoreBars();
			}
		...
		private void AddGpuValidBar(PerfMeterMetricsSnapshot metrics)
		{
			_valueBuilder.Length = 0;
		...
		private void AppendCpuCoreBars()
		{
			PerfMeterCpuCoreLoadSnapshot[] loads = GetOverlayCpuCoreLoads(out int count);
			PerfMeterCpuCoreLoadAvailability availability = GetOverlayCpuCoreLoadAvailability();
			if (count <= 0 || availability != PerfMeterCpuCoreLoadAvailability.Available)
			{
				bool unavailable = availability == PerfMeterCpuCoreLoadAvailability.Unavailable || availability == PerfMeterCpuCoreLoadAvailability.Unsupported;
				AddBarMessage("CPU Cores", GetCpuCoreLoadMessage(availability), unavailable ? UnavailableColor : AccentColor);
				return;
			}

			int visibleCount = Math.Min(count, MaxCpuCoreRows);
			_history.AddCpuCoreSamples(loads, visibleCount);
			for (int i = 0; i < visibleCount && HasAvailableBarField(); i++)
			{
				PerfMeterCpuCoreLoadSnapshot load = loads[i];
		...
		private static string GetCpuCoreLoadMessage(PerfMeterCpuCoreLoadAvailability availability)
		{
			switch (availability)
			{
				case PerfMeterCpuCoreLoadAvailability.WarmingUp:
					return "warming up";
				case PerfMeterCpuCoreLoadAvailability.Unavailable:
					return "unavailable";
				case PerfMeterCpuCoreLoadAvailability.Unsupported:
					return "unsupported";
				default:
					return "warming up";
			}
		...
		private void AddStatBar(StringBuilder name, double current, PerfMeterHistorySeries series, bool available, Color accent, PerfMeterStatBarFormat format)
		{
			if (!HasAvailableBarField())
			{
				return;
		...
		{
			AddStatBar(name, null, current, stats, available, accent, format);
		}

		private void AddStatBar(StringBuilder name, double current, PerfMeterHistoryStats stats, bool available, Color accent, PerfMeterStatBarFormat format)
		{
			AddStatBar(null, name, current, stats, available, accent, format);
		}

		private void AddStatBar(string name, StringBuilder nameBuilder, double current, PerfMeterHistoryStats stats, bool available, Color accent, PerfMeterStatBarFormat format)
		{
			if (!HasAvailableBarField())
			{
				return;
		...
		private PerfMeterCpuCoreLoadSnapshot[] GetOverlayCpuCoreLoads(out int count)
		{
			...
			{
				count = 0;
				return _cpuCoreLoadScratch;
			}

			PerfMeterCpuCoreLoadSnapshot[] loads = runtime.PeekCpuCoreLoads();
			count = Mathf.Clamp(runtime.CpuCoreLoadCount, 0, Math.Min(loads.Length, _cpuCoreLoadScratch.Length));
			for (int i = 0; i < count; i++)
			{
				_cpuCoreLoadScratch[i] = loads[i];
			}

			return _cpuCoreLoadScratch;
		}

		private static PerfMeterCpuCoreLoadAvailability GetOverlayCpuCoreLoadAvailability()
		{
			PerfMeterRuntime runtime = PerfMeterRuntime.Instance;
			return runtime != null ? runtime.CpuCoreLoadAvailability : PerfMeterCpuCoreLoadAvailability.Unsupported;
		}
		...
		private void AppendStatBarCurrent(StringBuilder builder, double value, PerfMeterStatBarFormat format)
		{
			...
			switch (format)
			{
				...
				case PerfMeterStatBarFormat.Megabytes:
					builder.Append(" MB");
					break;
				case PerfMeterStatBarFormat.Percent:
					builder.Append('%');
					break;
			}
		}
		...
		private void AppendStatBarValue(StringBuilder builder, double value, PerfMeterStatBarFormat format)
		{
			switch (format)
			{
				...
				case PerfMeterStatBarFormat.Ratio:
					...
					break;
				case PerfMeterStatBarFormat.Percent:
					AppendNonNegativeFixed(builder, value, "0.0");
					break;
				default:
					if (IsFinite(value))
					{
		...
		private sealed class PerfMeterStatBarField
		{
			...
			internal PerfMeterStatBarField(string name, Font nameFont, Font valueFont, Font statsFont)
			{
				...
				_fill = CreateMarkerElement(3f, 0f, 1f);
				_fill.style.left = 0f;
				_fill.style.width = Length.Percent(0f);
				_averageMarker = CreateMarkerElement(7f, -2f, 1f);
				_onePercentMarker = CreateMarkerElement(9f, -3f, 2f);
				_pointOnePercentMarker = CreateMarkerElement(11f, -4f, 2f);
				_track.Add(_fill);
			...
			internal void SetValue(string name, StringBuilder nameBuilder, StringBuilder min, StringBuilder max, StringBuilder current, StringBuilder average, StringBuilder onePercentValue, StringBuilder pointOnePercentValue, float currentPercent, float averagePercent, float onePercent, float pointOnePercent, Color accent)
			{
				...
				SetMarker(_averageMarker, averagePercent, WithAlpha(TextColor, 0.92f));
				SetMarker(_onePercentMarker, onePercent, WithAlpha(WarningColor, 0.98f));
				SetMarker(_pointOnePercentMarker, pointOnePercent, WithAlpha(new Color(1f, 0.12f, 0.24f, 1f), 0.98f));
		...
		internal sealed class PerfMeterFrameTimeStripElement : VisualElement
		{
			...
			private readonly CustomMetricSeries[] _customSeries = new CustomMetricSeries[PerfMeterOverlayPresetUtility.MaxCustomMetricGraphSeries];
			private int _customSeriesCount;
			private double _frameBudgetMs = PerfMeterCollector.DefaultFrameBudgetMs;
			private bool _drawFrameTime = true;
			private bool _drawCustomMetrics = true;
			...
			internal PerfMeterFrameTimeStripElement(int historyCapacity)
			{
				...
				for (int i = 0; i < _customSeries.Length; i++)
				{
			...
			internal int CustomSeriesCount => _customSeriesCount;
			internal double FrameBudgetMs => _frameBudgetMs;
			internal double PeakFrameTimeMs => _history.GetPeak();

			internal void SetRenderChannels(bool drawFrameTime, bool drawCustomMetrics)
			{
				_drawFrameTime = drawFrameTime;
				_drawCustomMetrics = drawCustomMetrics;
				MarkDirtyRepaint();
			}
			...
			internal void SetHistoryCapacity(int historyCapacity)
			{
				...
				for (int i = 0; i < _customSeries.Length; i++)
				{
					_customSeries[i].SetHistoryCapacity(normalized);
				}
				MarkDirtyRepaint();
			}

			internal void ConfigureCustomMetricSeries(PerfMeterCustomMetricGraphJson[] configurations)
			{
				PerfMeterCustomMetricGraphJson[] safeConfigurations = configurations ?? Array.Empty<PerfMeterCustomMetricGraphJson>();
				int configuredCount = 0;
				for (int i = 0; i < safeConfigurations.Length && configuredCount < _customSeries.Length; i++)
				{
					if (!PerfMeterOverlayPresetUtility.TryNormalizeCustomMetricGraph(safeConfigurations[i], out PerfMeterCustomMetricGraphConfiguration configuration) || ContainsCustomMetricId(configuration.MetricId, configuredCount))
					{
						continue;
					}

					_customSeries[configuredCount].Configure(configuration);
					configuredCount++;
				}

				for (int i = configuredCount; i < _customSeries.Length; i++)
				{
					_customSeries[i].Disable();
				}

				_customSeriesCount = configuredCount;
				MarkDirtyRepaint();
			}

			internal bool TryGetCustomSeriesConfiguration(int series, out PerfMeterCustomMetricGraphConfiguration configuration)
			{
				configuration = default;
				if (series < 0 || series >= _customSeriesCount)
				{
					return false;
				}

				configuration = _customSeries[series].Configuration;
				return true;
			}

			internal bool TryGetCustomSeriesSample(int series, int sample, out double value, out bool valid)
			{
				value = 0d;
				valid = false;
				return series >= 0 && series < _customSeriesCount && _customSeries[series].TryGetSample(sample, out value, out valid);
			}

			internal bool TryGetCustomSeriesLatest(int series, out double value, out bool valid)
			{
				value = 0d;
				valid = false;
				return series >= 0 && series < _customSeriesCount && _customSeries[series].TryGetLatest(out value, out valid);
			}
			...
			internal void AddCustomMetricSamples(int collectionFrame, PerfMeterCustomMetricSnapshot[] metrics, int count)
			{
				PerfMeterCustomMetricSnapshot[] safeMetrics = metrics ?? Array.Empty<PerfMeterCustomMetricSnapshot>();
				int safeCount = Mathf.Clamp(count, 0, safeMetrics.Length);
				bool changed = false;
				for (int i = 0; i < _customSeriesCount; i++)
				{
					changed |= _customSeries[i].AddSample(collectionFrame, safeMetrics, safeCount);
				}
			...
			private void OnGenerateVisualContent(MeshGenerationContext context)
			{
				...
				if (_drawCustomMetrics)
				{
					DrawCustomMetricSeries(painter, rect);
				}
			}
			...
			private bool ContainsCustomMetricId(string metricId, int configuredCount)
			{
				...
				{
					if (string.Equals(_customSeries[i].Configuration.MetricId, metricId, StringComparison.Ordinal))
					{
						return true;
					}
				}
			...
			private void DrawCustomMetricSeries(Painter2D painter, Rect rect)
			{
				...
				{
					CustomMetricSeries customSeries = _customSeries[series];
					int columnCount = Math.Min(customSeries.SampleCount, Mathf.Max(1, Mathf.FloorToInt(rect.width)));
					if (columnCount <= 0)
					{
						continue;
					}
					...
					painter.strokeColor = customSeries.Configuration.Color;
					for (int column = 0; column < columnCount; column++)
					{
						if (!customSeries.TryGetEnvelope(column, columnCount, out double min, out double max))
						{
							continue;
						}

						float x = rect.xMin + (column + 0.5f) * columnWidth;
						float maxY = CustomValueToY(rect, max, customSeries.Configuration);
						float minY = CustomValueToY(rect, min, customSeries.Configuration);
						painter.BeginPath();
						painter.MoveTo(new Vector2(x, maxY));
						painter.LineTo(new Vector2(x, Mathf.Min(rect.yMax, Mathf.Max(maxY + 1.5f, minY))));
						painter.Stroke();
			...
			{
				double normalized = (value - configuration.Min) / (configuration.Max - configuration.Min);
				return rect.yMax - Mathf.Clamp01((float)normalized) * rect.height;
			}
			...
			private sealed class CustomMetricSeries
			{
				private readonly PerfMeterFrameTimeStripHistory _history;

				internal CustomMetricSeries(int historyCapacity)
				{
					_history = new PerfMeterFrameTimeStripHistory(historyCapacity, requirePositive: false);
				}

				internal PerfMeterCustomMetricGraphConfiguration Configuration { get; private set; }
				internal int SampleCount => _history.Count;

				internal void Configure(PerfMeterCustomMetricGraphConfiguration configuration)
				{
					if (!HasSameConfiguration(configuration))
					{
						_history.Clear();
					}
				...
				{
					if (!string.IsNullOrEmpty(Configuration.MetricId))
					{
						_history.Clear();
						Configuration = default;
					}
				...
				internal void SetHistoryCapacity(int historyCapacity)
				{
					_history.SetCapacity(historyCapacity);
				}

				internal bool AddSample(int collectionFrame, PerfMeterCustomMetricSnapshot[] metrics, int count)
				{
					double value = 0d;
					bool valid = false;
					for (int i = 0; i < count; i++)
					{
						PerfMeterCustomMetricSnapshot metric = metrics[i];
						if (!string.Equals(metric.Id, Configuration.MetricId, StringComparison.Ordinal))
						{
							continue;
						}

						value = metric.Value * Configuration.DisplayScale;
						valid = metric.Available && !double.IsNaN(value) && !double.IsInfinity(value);
						break;
					}

					return _history.AddSample(collectionFrame, value, valid);
				}

				internal bool TryGetSample(int sample, out double value, out bool valid)
				{
					return _history.TryGetSample(sample, out _, out value, out valid);
				}

				internal bool TryGetLatest(out double value, out bool valid)
				{
					return _history.TryGetLatest(out value, out valid);
				}

				internal bool TryGetEnvelope(int column, int columnCount, out double min, out double max)
				{
					return _history.TryGetEnvelope(column, columnCount, out min, out max);
				}

				private bool HasSameConfiguration(PerfMeterCustomMetricGraphConfiguration configuration)
				{
					return string.Equals(Configuration.MetricId, configuration.MetricId, StringComparison.Ordinal) &&
						Configuration.Min.Equals(configuration.Min) &&
						Configuration.Max.Equals(configuration.Max) &&
						Configuration.DisplayScale.Equals(configuration.DisplayScale) &&
						string.Equals(Configuration.ColorText, configuration.ColorText, StringComparison.Ordinal) &&
						string.Equals(Configuration.Unit, configuration.Unit, StringComparison.Ordinal);
				}
			}
		}
		...
		private sealed class PerfMeterOverlayHistory
		{
			...
			internal readonly PerfMeterHistorySeries[] CpuCoreLoadPercent = CreateCpuCoreHistory();
			...
			internal void AddCpuCoreSamples(PerfMeterCpuCoreLoadSnapshot[] loads, int count)
			{
				int safeCount = Math.Min(count, CpuCoreLoadPercent.Length);
				for (int i = 0; i < safeCount; i++)
				{
					CpuCoreLoadPercent[i].Add(loads[i].LoadPercent, loads[i].Available);
				}
			}

			private static PerfMeterHistorySeries[] CreateCpuCoreHistory()
			{
				PerfMeterHistorySeries[] series = new PerfMeterHistorySeries[MaxCpuCoreRows];
				for (int i = 0; i < series.Length; i++)
				{
					series[i] = new PerfMeterHistorySeries();
				}
