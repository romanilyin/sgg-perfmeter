...
These notes keep the detailed local workflow that used to live in the root `AGENTS.md`. The root `AGENTS.md` is now kept short and public-facing for external agents.
...
- Read `_DevelopmentDocs/README.md`, `_DevelopmentDocs/decisions/`, and relevant `_DevelopmentDocs/backlog/` files before changing profiler architecture.
- Chat with the user in Russian.

## Package Layout

- Profiler package is `Assets/Scripts/SGG.PerfMeter/` with populated `Runtime/`, `Editor/`, `Tests/`, `Samples~`, and short package-local `.Documentation/` directories.
...
- There are no repo-local build/test/lint scripts; validation normally requires Unity editor/batchmode for this project.
...
- Avoid uGUI/IMGUI runtime overlays for this profiler; use low-overhead UI Toolkit plus URP Render Graph integration.
