...
- Runtime settings now support project-owned JSON zero-code setup at `Assets/Resources/SGG.PerfMeter/perfmeter-settings.json`; runtime loads it as `SGG.PerfMeter/perfmeter-settings`, auto-starts only when `enabled` and `autoStart` are true, and exposes the loaded/default settings through `PerformanceMeter.GetSettings()`.
...
- Editor setup window now also has a `Presets` tab for JSON settings and zero-code setup; `ScriptableObject` settings are intentionally not used.
