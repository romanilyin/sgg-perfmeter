...
- `PerformanceMeter.StartSession(...)`, `StopSession()`, `GetSessionSummary()`, and `IsSessionRecording` provide the session recorder core: bounded in-memory samples, start metadata from settings/device/camera snapshots, summary timing/FPS/bottleneck/spike counts, and dropped-sample reporting. JSON/CSV export is intentionally deferred.
...
- Tests include EditMode API/classifier/session coverage and PlayMode runtime smoke coverage for overlay lifecycle, snapshot updates, session recording, heatmap toggles, and overdraw terminal/degraded states.
