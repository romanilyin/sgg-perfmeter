...
- Runtime overlay corner placement is configurable through `PerformanceMeter.SetOverlayCorner(...)`; display mode is configurable through `PerformanceMeter.SetOverlayMode(...)` with `FpsOnly`, `TextCompact`, `Graphs`, and `Full`; JSON-backed overlay presets/modules are configurable through `PerformanceMeter.SetOverlayPreset(...)`, `PerformanceMeter.SetOverlayModules(...)`, and `PerformanceMeter.SetOverlayModuleVisible(...)`; target line budget is configurable through `PerformanceMeter.SetTargetFps(...)` for 15/30/60/90/120/144/240 FPS. Default placement is `TopRight`/`FullDiagnostics`/`Full`/`60 FPS`, with module-filtered stable text fields, cached enum strings, dirty value-label assignment, reusable numeric formatting buffers, JSON-tunable scale/opacity/font/refresh/history, stacked CPU graph layout, softer colored graph legends, fixed-width graph numbers, placeholder formatting for unavailable samples/counters, text min/max history, and generated UI Toolkit theme/text settings for reliable runtime text rendering.
- Runtime settings now support project-owned JSON zero-code setup at `Assets/Resources/SGG.PerfMeter/perfmeter-settings.json`; runtime loads it as `SGG.PerfMeter/perfmeter-settings`, auto-starts only when `enabled` and `autoStart` are true, stores collection mode, built-in overlay presets/active module lists, overlay tunables, alert thresholds/consecutive frame windows/cooldowns, session defaults, and overdraw default/max frame counts, and exposes the loaded/default settings through `PerformanceMeter.GetSettings()`.
...
- Editor setup window now also has a `Presets` tab for JSON settings, collection mode, active overlay preset selection, module toggles, overlay/rule/session/overdraw tunables, and zero-code setup; `ScriptableObject` settings are intentionally not used.
...
- Iteration 40 JSON tunables verification passed with `Logs/opencode-iter40-json-tunables-compile.log`, `Logs/opencode-iter40-json-tunables-editmode-results.xml`, and `Logs/opencode-iter40-json-tunables-playmode-results.xml`.
...
- In Play Mode, open Unity Profiler with the GC Alloc column enabled and sample `PerfMeterOverlay.RefreshText` in `FpsOnly`, `TextCompact`, `Graphs`, and `Full` modes.
- Verify the text block no longer rebuilds a single large managed string each refresh; unchanged field rows should not assign new `Label.text` values.
- Treat allocations from changed numeric values or graph legend labels as expected remaining work unless they grow every frame or bypass the `RefreshIntervalSeconds` throttle.
