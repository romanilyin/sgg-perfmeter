...
- Android player smoke validation helpers now exist outside the package: `PerfMeterAndroidBuild.BuildDevelopmentApk` builds a Development APK after recommended setup, and `PerfMeterAndroidSmokeBootstrap` runs only in Android players to emit `SGG_PERFMETER_SMOKE` logcat markers.
...
- Broader target-device validation is still useful for GLES/fallback behavior and platform-specific ProfilerRecorder counter availability.
