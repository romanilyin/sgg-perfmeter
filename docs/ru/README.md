...
- **Публичный C# API**: неизменяемые снимки status, metrics, device, camera, Render Graph, alerts/оповещения, sessions и custom metrics.
...
- Снимки device, camera, Render Graph, status, metrics, alerts/оповещения, session и custom metrics для кода и автоматизации через MCP.
...
Unity от `2022.3` до `6000.3` может импортироваться для проверки компиляции, но оверлей во время выполнения, фичи Render Graph, overdraw passes и ожидаемая поддержка требуют Unity `6000.4+` с URP `17.4+`.
