...
- Координировать RenderDoc/PIX через generic-путь Unity или опционально использовать отдельно распространяемый и проверяемый по SHA-256 RenderDoc bridge для authenticated `.rdc` в Windows x64 Editor с D3D11, D3D12 или Vulkan. UPM-пакет остается без бинарников и не устанавливает RenderDoc.
...
- **Сбор и экспорт**: core использует ограниченный интервал опроса provider-а 0.25 секунды и кэшированный snapshot с last-attempt/result, возрастом last-success и freshness. Capture boundary принудительно запрашивает sample, не скрывая результат `Unavailable`. Session JSON/CSV и capture samples сохраняют snapshot и provenance provider-а. MCP-команда `perfmeter.platform.telemetry` отдает текущий structured snapshot.
