...
`FPS Only` - это режим пресета/компоновки, а не отдельный виджет. Он использует FPS и данные таймингов в одной компактной строке.
...
| `timing.frame-spikes-card` | <img src="../assets/screenshots/widgets/timing-frame-spikes-card.png" alt="Frame spikes card" width="360"> | Карточка | FPS / Warnings | Счетчики frame spikes и текущее состояние предупреждения. |
...
- Text-row и metric-bar rows являются низкоуровневыми renderers внутри системы компоновки и показывают текстовые версии тех же групп метрик, которые в других режимах компоновки могут выглядеть как карточки, полосы бюджета или графики.
