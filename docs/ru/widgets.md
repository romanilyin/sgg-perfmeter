...
SGG PerfMeter включает 16 высокоуровневых виджетов оверлея во время выполнения. Это блоки композиции, которые видны в окне настройки и используются визуальными пресетами оверлея.

`FPS Only` - это режим пресета и компоновки, а не отдельный виджет. Он использует FPS и данные таймингов в одной компактной строке.

Почти у всех групп метрик есть текстовая и графическая форма. Текстовая форма - это карточки или строки `MetricBars` с числовыми значениями, а графическая форма - полосы бюджета или графики истории. Пресет выбирает, какую форму показать; один и тот же источник данных может отображаться в разных режимах компоновки.

Текст оверлея во время выполнения не локализуется, поэтому документация на разных языках использует одни и те же скриншоты виджетов.
...
| `timing.frame-spikes-card` | <img src="../assets/screenshots/widgets/timing-frame-spikes-card.png" alt="Frame spikes card" width="360"> | Карточка | FPS / Warnings | Счетчики frame spikes и текущее состояние предупреждений. |
| `overdraw.card` | <img src="../assets/screenshots/widgets/overdraw-card.png" alt="Overdraw card" width="360"> | Карточка | Overdraw / Heatmap | Состояние измерения overdraw, progress, ratio и состояние heatmap; в HDRP может показывать unsupported state для overdraw/heatmap. |
| `timing.cpu-budget-bar` | <img src="../assets/screenshots/widgets/timing-cpu-budget-bar.png" alt="CPU budget bar" width="360"> | Полоса бюджета | Timing | CPU frame time относительно бюджета выбранного целевого FPS. |
| `timing.gpu-budget-bar` | <img src="../assets/screenshots/widgets/timing-gpu-budget-bar.png" alt="GPU budget bar" width="360"> | Полоса бюджета | GPU timing | GPU frame time относительно бюджета выбранного целевого FPS. |
...
| `cpu.cores-bars` | <img src="../assets/screenshots/widgets/cpu-cores-bars.png" alt="CPU core bars" width="360"> | Панель | CPU core sampling | Полосы загрузки CPU по логическим ядрам, когда доступен platform sampling. |
...
Компоновка `MetricBars` по умолчанию показывает компактные строки для часто отслеживаемых групп:
...
| <img src="../assets/screenshots/widgets/metric-bars-timing.png" alt="Timing metric bars" width="480"> | Строки CPU/GPU timing относительно выбранного целевого FPS. |
...
- Пресеты могут включать часть этих виджетов и выбирать компоновку: `MetricBars`, `CompactCards`, `Graphs` или `DiagnosticsWide`.
- Text-row и metric-bar rows - низкоуровневые renderers внутри системы компоновки. Они показывают текстовые версии тех же групп метрик, которые в других режимах компоновки могут выглядеть как карточки, полосы бюджета или графики.
- Числовые значения используют стабильные зарезервированные numeric slots и numeric monospace role, поэтому смена значения не сдвигает колонки; подписи сохраняют выбранный шрифт оверлея.
- При узкой logical width карточки и metric bars переносятся. `FpsOnly` использует детерминированный ограниченный двухрядный fallback, если одна строка не помещается; это ограниченное поведение компоновки, а не обещание для любой произвольной resolution или scale.
