...
Метаданные пакета используют Unity `2022.3` как import-safety floor для проверок импорта и компиляции. Актуальная поддерживаемая версия для использования во время выполнения начинается с Unity `6000.4+` с URP `17.4+` Render Graph.
