...
- GRD сообщает configured mode и public SRP support, но activity имеет `Unknown`: в Unity 17.4 и 17.5 различается enabled semantics. Более глубокая GRD telemetry относится к `PM-GRD-001`; этот snapshot не утверждает GRD activity.
