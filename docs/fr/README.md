...
- **Collecte et exports**: le core utilise une cadence bornee de 0,25 seconde et un snapshot en cache avec les metadonnees last attempt/result, l'age du last success et la freshness. Une capture boundary force une tentative sans masquer `Unavailable`. Le JSON/CSV de session et les capture samples preservent le snapshot et la provenance du provider. La commande MCP `perfmeter.platform.telemetry` expose le snapshot structure actuel.
