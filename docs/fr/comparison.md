...
SGG PerfMeter est volontairement plus cible et plus diagnostique: Unity `6000.4+`, URP `17.4+`, Render Graph, snapshots structures, export de session, diagnostics d'overdraw, metadonnees reproductibles de camera/device et automatisation MCP/API.
...
| Overdraw | 🔵 Mesure numerique + heatmap visuelle via URP Render Graph | 🛑 Non | 🛑 Non |
...
- Expose des compteurs de rendu orientes URP et des diagnostics Render Graph.
