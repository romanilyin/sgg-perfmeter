...
- `GenericUnity` usa l'`ExternalGPUProfiler` sperimentale di Unity in Editor/Development Build. La sua matrice resta RenderDoc su desktop Windows/Linux con D3D11/D3D12/Vulkan e PIX su desktop Windows con D3D12; completion non autentica tool o artefatto.
...
## Limiti del contesto di render integration

- `PerfMeterRenderIntegrationSnapshot` è un contratto di observation neutrale rispetto all'integrazione, non un capture profondo di Render Graph o Custom Pass. Le letture non avviano il runtime; prima della prima observation la pipeline corrente supportata può essere `Available` con `NotObserved`, mentre un cambio di pipeline/configuration marca l'observation precedente come stale con `ObservationMatchesCurrentPipeline: false`, frame/age espliciti e warning.
- URP usa il `UniversalRenderingData.renderingMode` pubblico del frame corrente e riporta i pass PerfMeter realmente programmati. HDRP riporta il `CustomPass` PerfMeter effettivo, ma l'effective rendering mode resta unavailable.
- La reflection privata/interna di pass e risorse Render Graph è stata rimossa. La facade legacy mantiene `registered_pass_count`, `merged_pass_count`, `transient_resource_count`, `imported_resource_count` e `aliased_resource_count` a `-1`, perché non esiste una stable public API che li esponga.
- L'attività GRD usa il risultato pubblico di `IGPUResidentRenderPipeline.IsGPUResidentDrawerEnabled()` e descrive lo stato runtime globale, non l'uso GRD di una camera o di un renderer specifico. Forward+ in URP è una observation del frame corrente; in HDRP l'availability di rendering mode/Forward+ resta `Unknown`.
- L'effectiveness GRD usa counter aggregati BRG di draw call/istanze con provenance esatta. Possono includere altri utenti di `BatchRendererGroup`, quindi non dimostrano partecipazione GRD per renderer. I valori unavailable o senza sample sono serializzati come `null`.
- VRS espone il supporto hardware autorevole di `SystemInfo`/`ShadingRateInfo`. Configurazione e attività restano `Unknown` finché un futuro typed adapter non le dimostra; non viene dichiarata attività VRS.
- Unity non espone un viewer pubblico stabile RenderGraph/CustomPass né una API per i pass target. PerfMeter non aggiunge quindi navigazione nell'Editor e non la promette.
- Lo schema del context di capture v1 conserva `render` e aggiunge `render_integration`; gli schemi JSON/CSV di sessione non cambiano. Il context di un capture esterno viene congelato al primo sample `Capturing`, non sostituito da letture successive.
- Evidenza finale PM-REN-001: main compile Unity `6000.4.12f1` superato; `PerformanceMeterApiTests` targeted `53/53`, `PerfMeterCaptureBundleTests` `15/15` e `PerformanceMeterPlayModeSmokeTests` `12/12`; full EditMode finale `215/215` e full PlayMode `16/16` superati. Focused review P1/P2 resolved. La compile matrix isolata è superata su Unity `6000.4.12f1` URP `17.4` e HDRP `17.4`, e Unity `6000.5.6f1` URP `17.5` e HDRP `17.5`. La validazione release-player/device resta pending; nessuna release è dichiarata.
- Evidenza finale PM-GRD-001: compile Unity `6000.4.12f1` superato; API targeted `58/58`, capture-bundle `15/15` e PlayMode smoke `12/12`; full EditMode `220/220` e PlayMode `16/16` superati. Focused review P1/P2 resolved; la compile matrix Unity `6000.4`/`6000.5` con URP `17.4`/`17.5` e HDRP `17.4`/`17.5` è superata. Il comportamento release-player/device resta pending.
