...
I componenti sono `Collector`, `CustomMetricProviders`, `CpuCoreProvider`, `Overlay`, `UrpRenderIntegration` e `HdrpRenderIntegration`. Ognuno espone conteggi di frame/invocazioni, millisecondi CPU medi/massimi, allocazioni totali/medie, budget e stati `NotEvaluated`/`WithinBudget`/`Exceeded`. La provenance additiva include epoch, primo/ultimo frame misurato, conteggio dei callback frame, motivo di inattivita tipizzato e disponibilita esplicita dell'attribuzione GPU.

`GetSelfOverheadWindow(...)` restituisce l'osservazione URP legata esattamente a una session o capture, con identita, epoch, limiti dei frame, containment ed evidence di quality/pipeline/renderer e feature installed/enabled/enqueued. I risultati inattivi usano motivi tipizzati come `RendererFeatureNotInstalled`, `RendererFeatureDisabled`, `PassNotEnqueued`, `NoCameraCallbackObserved`, `WindowIncomplete` o `CaptureWindowMismatch`; se manca evidence viene restituito `UnknownInactiveReason`. Una capture successiva non puo riutilizzare una epoch completata in precedenza.
...
Lo scope URP misura solo la registrazione CPU-side `RecordRenderGraph()` posseduta dal package e l'allocazione del current thread. Con piu camere le invocazioni possono superare i callback frame. L'attribuzione GPU e esplicitamente `Unavailable`; CPU, GPU, hitch e GC whole-frame restano contesto e non vengono attribuiti a PerfMeter per vicinanza temporale. Questi diagnostics non sottraggono ne modificano le metriche CPU/GPU esistenti.
...
`PerfMeterCaptureBackendMode.GenericUnity` resta il default di compatibilita per `ExternalGPUProfiler`; tool e artifact identity non sono autenticati. `NativePreferred` richiede il bridge opzionale Windows x64 Editor e puo fare fallback solo prima del native begin; `NativeRequired` non fa mai fallback. Il percorso nativo supporta D3D11, D3D12 e Vulkan. Lo status espone `RequestedBackendMode`, `EffectiveBackendKind`, `NativePhase`, result code e fallback reason.
...
I `.rdc`/`.wpix` generic o forniti dal caller restano observed. Solo il descriptor nativo generation-bound puo autenticare un `.rdc` finalizzato. Native MetadataOnly usa `DoNotShare`; Copy/Embed hanno quote separate e `ReviewBeforeShare`. Traversal, reparse points e file fuori dagli owned roots vengono rifiutati.

L'esportazione del capture bundle dispone anche di un'API single-flight non bloccante: `RequestCaptureBundleExport(..., out exportId)`, `GetCaptureBundleExportStatus(exportId)` e `CancelCaptureBundleExport(exportId)`. Lo stato riporta fase, avanzamento, byte, annullamento, nuovi tentativi, percorso di commit ed envelope generico dell'artefatto esterno. L'API esistente `ExportCaptureBundle(...)` rimane un wrapper di compatibilita bloccante, mentre serializzazione, I/O su file, hashing, retention e commit atomico vengono eseguiti in un worker thread.

I JSON di sessione e capture aggiungono eventi di timeline tipizzati per sample mancanti e limiti della capture. Le versioni dello schema, gli array di sample e le colonne CSV esistenti rimangono compatibili; i payload legacy o sconosciuti vengono letti senza inventare gap. I provider di custom metrics usano un provider snapshot in cache e un buffer riutilizzabile di proprieta del core nel warmed collection path; le copie vengono create solo per sample conservati, export e public snapshots. Il coordinamento del Profiler e locale al processo tramite `GetProfilerLeaseCapabilities()`, `GetProfilerLeaseStatus()`, `TryAcquireProfilerLease(...)` e `ReleaseProfilerLease(...)`; i lease detenuti non sopravvivono a un domain reload.
...
URP usa il `UniversalRenderingData.renderingMode` pubblico del frame corrente e riporta i pass PerfMeter realmente programmati per quel frame. HDRP riporta il `CustomPass` PerfMeter effettivamente osservato, ma l'effective rendering mode non è disponibile. `GpuResidentDrawer` riporta modalità configurata, supporto SRP/progetto/compute, Forward+ e compatibilità clustered del frame URP, oltre all'attività runtime globale da `IGPUResidentRenderPipeline.IsGPUResidentDrawerEnabled()`. In HDRP i campi Forward+/rendering mode restano `Unknown`. `VariableRateShading` riporta il supporto hardware autorevole di `SystemInfo`/`ShadingRateInfo`; configurazione e attività restano `Unknown` finché un typed adapter non le dimostra.
...
`GpuResidentDrawer` aggiunge `ProjectConfigurationAvailability`, `IsProjectConfigurationSupported`, `ComputeShaderAvailability`, `SupportsComputeShaders`, `ForwardPlusActivityAvailability`, `IsObservedForwardPlusActive`, `RenderingModeCompatibilityAvailability`, `IsRenderingModeCompatible`, `ActivitySource`, `DegradedReason` ed `Effectiveness`. `PerfMeterGpuResidentDrawerReason` fornisce stati di fallback strutturati. `PerfMeterGpuResidentDrawerEffectivenessSnapshot` contiene valori BRG di draw call/istanze e provenance delle capability Profiler; i valori senza sample sono `-1` in C# e `null` in JSON. Sono counter BatchRendererGroup aggregati, non evidence GRD autorevole per renderer.

## Correlazione Della Sessione

`PerformanceMeter.GetSessionSummary().SessionId` e un identificatore esadecimale minuscolo di 32 caratteri. Viene creato da `StartSession`, rimane stabile dopo `StopSession`, cambia quando inizia una nuova sessione ed e vuoto quando non esiste alcuna sessione. Il JSON della sessione espone lo stesso valore nel campo radice `session_id`; il CSV lo aggiunge come ultima colonna `session_id` per mantenere le posizioni esistenti; `perfmeter.session.summary` lo restituisce come `session_id`.
