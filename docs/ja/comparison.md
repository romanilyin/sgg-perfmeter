...
SGG PerfMeter は、より診断向けに範囲を絞っています。Unity `6000.4+`、URP `17.4+` Render Graph、HDRP `17.4+` Custom Pass diagnostics、structured snapshots、session export、URP overdraw diagnostics、reproducible camera/device metadata、MCP/API automation を対象にしています。
...
| Unity target | ⚠️ Unity `6000.4+`, URP `17.4+` / HDRP `17.4+` | 🔵 幅広い古い Unity support | 🔵 幅広い古い Unity support |
| UI backend | 🔵 PerfMeter 所有の UI Toolkit host: Unity `6000.4` では `UIDocument`、Unity `6000.5+` では `PanelRenderer` | ⚠️ uGUI Canvas/Text labels | ⚠️ uGUI Text/Image modules |
...
| Overdraw | 🔵 URP Render Graph 経由の numerical measurement + visual heatmap。HDRP では explicit unsupported state | 🛑 なし | 🛑 なし |
...
- URP-oriented render counters、URP Render Graph diagnostics、HDRP Custom Pass observation を公開します。
