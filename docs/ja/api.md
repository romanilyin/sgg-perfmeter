...
`PerfMeterCaptureBackendMode.GenericUnity` は `ExternalGPUProfiler` の compatibility default で、tool/artifact identity は authenticate されません。`NativePreferred` は optional Windows x64 Editor bridge を要求し、native begin 前だけ fallback できます。`NativeRequired` は fallback しません。Native path は D3D11、D3D12、Vulkan をサポートします。status は `RequestedBackendMode`、`EffectiveBackendKind`、`NativePhase`、result code、fallback reason を報告します。
...
Generic または caller-supplied `.rdc`/`.wpix` は observed のままです。generation-bound native descriptor だけが finalized `.rdc` を authenticate できます。Native MetadataOnly は `DoNotShare`、Copy/Embed は separate quota と `ReviewBeforeShare` を使用します。Traversal、reparse point、owned root 外の file は reject されます。
