...
- **収集と export**: core は 0.25 秒の bounded cadence と cache を所有し、last attempt/result、last success age、freshness を公開します。capture boundary では `Unavailable` を隠さずに sample を強制します。session の JSON/CSV と capture samples には snapshot と provider provenance が保持されます。MCP command `perfmeter.platform.telemetry` から現在の structured snapshot を取得できます。
