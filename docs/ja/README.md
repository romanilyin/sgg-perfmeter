...
**Unity 6 URP 向けの軽量なランタイム性能診断と、エージェントが読み取れるプロファイリング。**
...
- Unity `6000.4+` / URP `17.4+` Render Graph の runtime state。
...
- URP Render Graph 経由の opt-in numerical overdraw measurement と visual overdraw heatmap。
- code と MCP automation 向けの device、camera、Render Graph、status、metrics、alerts、session、custom metric snapshots。
...
- **Overdraw diagnostics**: URP renderer feature がインストールされている場合に、bounded numerical measurement を要求するか visual heatmap を有効化します。
...
Advanced FPS Counter と Graphy は、汎用の drop-in visual overlay として優れています。SGG PerfMeter は、modern Unity URP diagnostics に意図的に焦点を絞っています。対象は structured timing と render counters、bottleneck classification、reproducible sessions、device/camera snapshots、overdraw diagnostics、Render Graph state、MCP/API automation です。
...
Unity `2022.3` から `6000.3` は compile checks 用に import-safe な場合がありますが、runtime overlay、Render Graph features、overdraw passes、support expectations は Unity `6000.4+` と URP `17.4+` を対象にしています。`6000.4` より前のバージョンでは一部機能が動作しない場合があります。
