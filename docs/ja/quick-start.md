...
- editable active URP renderers に `PerfMeterRenderGraphFeature` をインストールします。または HDRP projects は変更せず、package Custom Pass が runtime で登録されます。
...
- URP Render Graph diagnostics または overdraw をテストする場合、active URP renderer に `PerfMeterRenderGraphFeature` があること。
- HDRP では setup が HDRP Custom Pass availability を報告することを確認します。HDRP overdraw と heatmap は by design unsupported です。
