...
- **Recopilación y exportación**: el core usa una cadencia limitada de 0,25 segundos y un snapshot en caché con metadatos de last attempt/result, antigüedad del last success y freshness. Un capture boundary fuerza un intento sin ocultar `Unavailable`. El JSON/CSV de sesión y los capture samples conservan el snapshot y la procedencia del provider. El comando MCP `perfmeter.platform.telemetry` expone el snapshot estructurado actual.
