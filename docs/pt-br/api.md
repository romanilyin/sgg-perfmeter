...
`PerfMeterCaptureBackendMode.GenericUnity` continua sendo o default de compatibilidade para `ExternalGPUProfiler`; tool e artifact identity nao sao autenticados. `NativePreferred` solicita o bridge opcional Windows x64 Editor e so pode fazer fallback antes do native begin; `NativeRequired` nunca faz fallback. O caminho nativo suporta D3D11, D3D12 e Vulkan. O status informa `RequestedBackendMode`, `EffectiveBackendKind`, `NativePhase`, result code e fallback reason.
...
Os `.rdc`/`.wpix` generic ou fornecidos pelo caller continuam observed. Somente o descriptor nativo generation-bound pode autenticar um `.rdc` finalizado. Native MetadataOnly usa `DoNotShare`; Copy/Embed tem quotas separadas e `ReviewBeforeShare`. Traversal, reparse points e arquivos fora dos owned roots sao rejeitados.
