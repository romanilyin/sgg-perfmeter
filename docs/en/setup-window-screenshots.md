...
- **RenderDoc**: keep RenderDoc user-installed, then optionally use **Download Verified Bridge** or **Install Local Bridge** for the exact separately published Windows x64 Editor artifact. FTUE shows cancel/remove/status and restart guidance, checks Unity's shared attachment signal, links the official guides, and copies a `NativeRequired` + `Copy` `RequestCapture` snippet. The package remains binary-free, never installs or loads RenderDoc, and supports native capture only on Windows x64 Editor D3D11/D3D12/Vulkan.
