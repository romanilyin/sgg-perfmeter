...
| `overdraw.card` | <img src="../assets/screenshots/widgets/overdraw-card.png" alt="Overdraw card" width="360"> | Card | Overdraw / Heatmap | Overdraw measurement state, progress, ratio, and heatmap state; in HDRP it can show unsupported state for overdraw/heatmap. |
...
| `graphs.raw-frame-time` | Runtime-only paired graphs | Graph | Graphs / Timing / Custom metrics | A dedicated raw frame-time strip with budget severity plus a separate graph for up to four configured stable-ID custom metric channels. Custom channels use independent explicit signed ranges, display scales, colors, units, and live numeric legends; missing or unavailable values produce gaps. |
