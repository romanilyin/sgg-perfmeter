...
SGG PerfMeter는 의도적으로 더 좁고 진단 중심입니다. Unity `6000.4+`, URP `17.4+`, Render Graph, structured snapshots, session export, overdraw diagnostics, 재현 가능한 camera/device metadata, MCP/API automation에 초점을 둡니다.
...
| Overdraw | 🔵 URP Render Graph를 통한 numerical measurement + visual heatmap | 🛑 없음 | 🛑 없음 |
...
- URP-oriented render counters 및 Render Graph diagnostics를 노출합니다.
