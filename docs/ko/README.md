...
- **수집 및 export**: core는 0.25초 bounded cadence와 cache를 소유하고 last attempt/result, last success age, freshness를 공개합니다. capture boundary에서는 `Unavailable`을 숨기지 않고 sample을 강제합니다. session JSON/CSV와 capture samples는 snapshot 및 provider provenance를 보존합니다. MCP command `perfmeter.platform.telemetry`로 현재 structured snapshot을 확인할 수 있습니다.
