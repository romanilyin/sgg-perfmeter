...
| `perfmeter.capture.request` | bounded GPU capture를 request합니다. optional `backend_mode`: `GenericUnity`, `NativePreferred`, `NativeRequired`. Native storage mode는 C# API에서만 선택합니다. |
