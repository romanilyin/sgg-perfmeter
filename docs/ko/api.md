...
`PerfMeterCaptureBackendMode.GenericUnity`는 `ExternalGPUProfiler` compatibility default이며 tool/artifact identity를 authenticate하지 않습니다. `NativePreferred`는 optional Windows x64 Editor bridge를 요청하고 native begin 전까지만 fallback할 수 있습니다. `NativeRequired`는 fallback하지 않습니다. Native path는 D3D11, D3D12, Vulkan을 지원합니다. status는 `RequestedBackendMode`, `EffectiveBackendKind`, `NativePhase`, result code, fallback reason을 보고합니다.
...
Generic 또는 caller-supplied `.rdc`/`.wpix`는 observed로 유지됩니다. generation-bound native descriptor만 finalized `.rdc`를 authenticate할 수 있습니다. Native MetadataOnly는 `DoNotShare`, Copy/Embed는 separate quota와 `ReviewBeforeShare`를 사용합니다. Traversal, reparse point, owned root 외부 file은 reject됩니다.
