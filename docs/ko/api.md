...
Device snapshot에는 Unity/platform/OS/CPU/GPU/API/display/window/support 정보가 포함됩니다. Camera snapshot에는 사용 가능한 경우 scene, transform, projection, clipping, pixel rect, target display, URP camera settings가 포함됩니다.
...
Overdraw diagnostics는 명시적인 diagnostic mode이며 GPU work를 추가할 수 있습니다.
