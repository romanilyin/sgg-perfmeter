...
- Native zirkulaere PIX-Timing-Captures sind nicht verfuegbar. Die dokumentierte Windows-Timing-API von Microsoft unterstuetzt Forward-Captures, ignoriert aber Steuerungen fuer zirkulaeren Speicher, Speicherlimit und Verwerfen; PerfMeter ersetzt den angeforderten Pre-Alert-Ring weder durch einen Forward-Capture ohne dokumentiertes Speicherlimit noch durch eine private PIX-Integration.
...
- Die GRD-Aktivitaet verwendet das oeffentliche Ergebnis von `IGPUResidentRenderPipeline.IsGPUResidentDrawerEnabled()` und beschreibt den globalen Runtime-Zustand, nicht die Nutzung durch eine bestimmte Kamera oder einen Renderer. URP-Forward+ ist eine aktuelle Frame-Observation; bei HDRP bleibt die Rendering-Modus-/Forward+-Availability `Unknown`.
- GRD-Effectiveness verwendet aggregierte BRG-Draw-Call-/Instanz-Counter mit exakter Capability-Provenienz. Andere `BatchRendererGroup`-Nutzer koennen enthalten sein; daraus folgt keine GRD-Teilnahme pro Renderer. Nicht verfuegbare oder noch nicht gesampelte Werte werden als `null` serialisiert.
...
- PM-GRD-001-Finalevidenz: Unity `6000.4.12f1` Compile bestanden; targeted API `58/58`, Capture-Bundle `15/15` und PlayMode Smoke `12/12`; Full EditMode `220/220` und PlayMode `16/16` bestanden. Focused Review P1/P2 resolved; die Unity-`6000.4`/`6000.5`-Compile-Matrix fuer URP `17.4`/`17.5` und HDRP `17.4`/`17.5` ist bestanden. Release-Player-/Geraeteverhalten bleibt pending.
